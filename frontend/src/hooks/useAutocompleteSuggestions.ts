// From https://github.com/visgl/react-google-maps/blob/main/examples/autocomplete/src/hooks/use-autocomplete-suggestions.ts
...
function useAutocompleteSuggestions(
  ...
  useEffect(() => {
    ...
    AutocompleteSuggestion.fetchAutocompleteSuggestions(request).then((res) => {
      const predictions = res.suggestions.filter(
